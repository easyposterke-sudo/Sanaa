...
- Check fitted single-portrait height using both asset dimensions and canvas dimensions. Request enlargement and reflow before compilation when it is below 52%; aim for 60–75%. Explicit small portraits, framed portraits and groups remain exempt.
...
- Validate required facts and composition before compilation. Review the rendered first draft. If a review drops facts or fails geometry checks, retry once with the failed checks and the original valid manifest/screenshot. Never apply the rejected manifest. Network failures retain the validated draft with an explicit incomplete-review notice.
...
Validation includes fitted-aspect-ratio checks, preserved blending, date/time separation, and a component regression proving that a review which drops Host is retried before being applied. Live model output still needs an end-to-end visual check with configured credentials.
