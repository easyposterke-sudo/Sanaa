...
const styles = ['Warm split layout', 'Blue central speaker', 'Black and gold', 'White and maroon', 'Blue information cards', 'Pink typographic', 'Orange and olive', 'Red and blue speaker group', 'Green editorial theme', 'Purple layered occasion', 'Purple gold framed logistics', "Warm schedule and right speaker", "Diagonal maroon date sidebar", "Dark luminous title and portrait", "Orange blue circular backdrop", "White amber scripture layout", "Minimal Bible and warm bokeh", "Turquoise expressive encounter", "Navy cream sweeping paths"];
...
export function PosterPromptCreator({ onApply, onClose, onImport }: Props) {
  ...
  async function generate() {
    ...
    try {
      ...
      const families = portraits.length > 1 ? [8] : portraits.length ? [2, 4, 5, 8, 9, 10, 11, 12, 13, 14, 15, 16, 18, 19] : [1, 3, 6, 7, 17];
