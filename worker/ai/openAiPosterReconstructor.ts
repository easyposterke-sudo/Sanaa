...
- pathPoints are normalized 0..1 inside the element box and follow the visible boundary in order. Use 3 to 8 anchors for closed fills, or 2 to 8 for open strokes. Place anchors at edge intersections, extrema, and changes of curvature; preserve every major inward and outward bend, including near the top and bottom of the poster. Do not replace a curved boundary with a vertical or diagonal straight edge.
- For accurate curves, provide handleIn and handleOut as absolute normalized control-point coordinates relative to the SAME element box, never offsets from the anchor. The segment from anchor A to B uses A.handleOut and B.handleIn as cubic Bezier controls. Handles may extend outside the box within -1..2 when needed, but the resulting visible curve should follow the reference. Use independent lengths to match asymmetric bends; at a continuous smooth join keep the incoming and outgoing directions collinear and opposite. Inspect the curve between anchors, not just the anchor positions.
- When either handle on an anchor is provided, it overrides automatic smoothing at that anchor; a null opposite handle stays at the anchor. To make one side straight and the other curved, use a control at the anchor for the straight side. Set both handles null to retain automatic symmetric handles for smooth true, or a sharp corner for smooth false. Keep edge-aligned closing segments straight. Use pathTension only for automatic handles. Recheck the complete silhouette and its filled side before returning the path.
