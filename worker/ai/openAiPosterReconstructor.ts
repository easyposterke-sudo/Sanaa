...
- Final detail checklist: first verify that every readable word is represented by text and no image_region contains headline or factual copy. Then re-check image masks, cutout/fade treatment and direction, open_stroke versus closed_fill classification, native regular-shape classification, transparent fills and outlines, rectangle corners, font family, tight text bounds, visible line count, font size, character spacing, and alignment. Confirm that every boundary separating differently colored regions is a closed filled shape and that no circle, ellipse, rectangle, triangle, star, or straight line was emitted as a path. This check must not change correct layer order.
