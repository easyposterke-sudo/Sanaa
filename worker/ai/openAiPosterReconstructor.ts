...
- Detect condensed or expanded typography from geometry, not only from a font-family guess. The tight box width and visible glyph height must describe the final rendered ink even when the reference has been horizontally compressed or stretched. In particular, do not shorten a tall condensed headline merely to preserve a chosen font's natural aspect ratio.
...
- Final detail checklist: first verify that every readable word is represented by text and no image_region contains headline or factual copy. Then re-check large color-region coverage, image masks, cutout/fade treatment and direction, open_stroke versus closed_fill classification, native regular-shape classification, solid versus gradient vector fills, transparent fills and outlines, rectangle corners, font family, tight final rotated text bounds, visible line count, font size, character spacing, textCurve, and alignment. Confirm that every boundary separating differently colored regions is a closed filled shape and that no circle, ellipse, rectangle, triangle, star, or straight line was emitted as a path. This check must not change correct layer order.
