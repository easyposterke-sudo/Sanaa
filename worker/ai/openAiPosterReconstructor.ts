...
- Never recommend background removal for background_photo or person portraits. The editor's remover is a user-triggered objects-and-products tool; a clean isolated foreground object or product may be suitable, but reconstruction must never trigger removal automatically.
...
- Measure fontSizeRatio from the actual visible glyph height relative to the complete poster height; do not assign size from semantic importance or a generic heading/body preset. For multiple lines, cross-check that fontSizeRatio together with lineHeight and visibleLineCount reproduces the detected text box height. Compare nearby text blocks so their relative size hierarchy matches the reference exactly.
