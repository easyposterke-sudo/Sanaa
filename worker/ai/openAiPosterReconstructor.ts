...
- Detect every organization logo as a logo image_region even when its source crop looks clean. The editor will request an uploaded original logo and fit the whole image in that box. Do not ask for or construct a crop of the flattened poster; never merge an ordinary nearby church name or photo into a standalone emblem's logo box.
...
- Preserve simple multi-part symbols as their actual parts. For example, a plain Christian cross is a narrow upright bar and a shorter horizontal bar, with the surrounding negative space left open; do not replace its silhouette with one solid bounding rectangle. Use an image_region only when the symbol contains detailed photographic or branded artwork.
