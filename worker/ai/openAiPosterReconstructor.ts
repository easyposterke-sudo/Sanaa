...
- Inspect every rectangular panel, badge, button, and text background for corner treatment. For a rect, cornerRadiusRatio is the visible radius divided by the rectangle's shorter side: use 0 only for genuinely sharp corners, roughly 0.08 to 0.25 for softly rounded corners, and 0.5 for pill-shaped ends. Do not flatten visibly curved ends into square rectangles.
- If a rectangular panel has clearly different corner radii or only selected corners rounded, represent its visible boundary as a closed path instead of losing that asymmetry in a rect.
