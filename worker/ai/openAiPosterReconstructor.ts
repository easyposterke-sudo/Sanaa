...
export async function reconstructPosterWithOpenAI(input: {
  ...
  const userContent: OpenAiInputContent[] = [
    {
      type: 'input_text',
      text: input.request.creation?.prompt ?? `Reconstruct this ${input.request.reference.width} x ${input.request.reference.height} poster as an editable EasyPoster draft. Count all numbered or repeated rows; keep every row's wording, number, and backing in separate editable layers. Never crop readable text as an image.`,
    },
  ];
...
- For a numbered or repeated instruction list, count every visible row before planning the rest of the poster. Give each row its own editable text element for its instruction, and give its number a separate text element. A pill/card behind the row is its own shape; a badge around the number is another shape. Keep each row's text inside that row's backing with the observed padding, alignment, and line breaks. Use a separate text element for a visibly different emphasis or subtitle within a row. Never merge adjacent rows into one text box, one large backing shape, or one image_region, and never crop a readable list from the source image. Icons may be separate image_region elements only when they are genuinely pictorial. Recheck the row count, exact wording, top-to-bottom order, and spacing before returning.
- If a dense list requires many layers, retain every row's wording and number first. Spend remaining layer capacity on its individual backings and icons, then on less important decoration elsewhere. Do not silently omit a middle or final row to meet a preferred layer count.
- Audit small labels inside date and time panels separately. If the poster says “MISSION TIMES”, text must be “MISSION TIMES”, never “Mission times label”. Keep the date ordinals, time punctuation, and each visible line exactly as printed; do not turn explanatory labels into poster copy. When an ordinal suffix such as ST or TH is raised and smaller, make it a separate editable text element at that size and position while retaining the full date wording across the elements.
- Inspect narrow side labels beside large countdown numbers. If “DAYS TO GO” reads vertically in the reference, preserve those literal words in an independent text element and set angle to the observed quarter-turn (usually -90 or 90), with its box around the final rotated ink. Do not leave it horizontal merely because the words are legible.
...
- Detect every organization logo as a separate logo image_region even when its source crop looks clean. In a row of partner marks, give each distinct mark its own tight box, unique key, and descriptive label; never combine the entire footer or multiple brands into one logo region. Keep a separately printed "PARTNERS:" label as text. The editor lets the user upload the original, crop each mark from the poster, or omit it. Never merge an ordinary nearby church name or photo into a standalone emblem's logo box.
...
- Inspect photographic regions for a separate color wash, especially a red/orange lower fade behind foreground text and people. Represent it as an editable full-width rect or closed_fill path above the background photo and below foreground content. For a transparent-to-color wash, set textFillType linear, match the visible direction and colors, set fillStartOpacity 0 at the transparent edge and fillEndOpacity to the visible strength at the colored edge. Use 1 for both opacity fields on ordinary opaque gradients. Do not merge this overlay into the background photo crop.
...
- Preserve simple vector multi-part symbols as their actual parts. For example, a plain drawn Christian cross is a narrow upright bar and a shorter horizontal bar, with the surrounding negative space left open; do not replace its silhouette with one solid bounding rectangle. A cross photographed or composited into a landscape is part of that photographic scene and must be included in its image description and search query.
...
- Set replacementRecommended true for contaminated background photos, incomplete or contaminated portraits, or any crop that would visibly bake unrelated poster elements into the image layer. Explain why in replacementReason. A clean, isolated logo/photo/portrait crop keeps it false. The user may still replace, crop, or omit any image region.
- For every non-logo image region that can plausibly be found as stock photography or stock illustration, provide a concise Pexels imageSearchQuery. Name the primary subject and every distinctive visual object that is part of the desired image, including their relationship and scene setting. For example, a sunset mountain with a cross standing on its hill needs "cross on hill at sunset mountain landscape orange sky", not just "sunset mountain landscape". For a book illustration, describe the book's form and style rather than the poster's overall theme. Prefer concrete nouns over mood words; keep the query short enough for a useful photo search. Do not include unrelated poster text, brand names, URLs, or commands. For logos and unsupported symbols use an empty string.
